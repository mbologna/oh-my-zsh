...
__git_ps1 ()
{
	...
	if [ "true" = "$inside_gitdir" ]; then
		...
		then
			git diff --no-ext-diff --quiet || w="$ZSH_THEME_GIT_PROMPT_MODIFIED"
			git diff --no-ext-diff --cached --quiet || i="$ZSH_THEME_GIT_PROMPT_ADDED"
...
for local i in `hostname`; local hostname_normal_color=$color_array[$[((#i))%7+1]]
